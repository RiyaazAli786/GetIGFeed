...
/**
 * signer.js — loads iGram's request-signing chunk inside a minimal browser shim
 * and returns the signing function it exports.
 */
...
async function instantiate(source) {
  ...
  const patchedSource = chunk.patchChunkSource(source);

  vm.createContext(sandbox);
  vm.runInContext(patchedSource, sandbox, { filename: 'live_link_chunk.js' });

  const ids = Object.keys(modules);
  if (!ids.length) throw new Error('chunk did not register any modules');

  const __webpack_require__ = makeRequire(modules);
  const mod3508 = modules[3508] || modules['3508'] ? __webpack_require__(3508) : null;
  let signerPromise = mod3508 && mod3508.default;
  ...
  // Verify that it signs a test input correctly
  const probe = await signer({ username: 'instagram' });
...
async function load(refresh) {
  ...
  for (const source of chunk.sources({ refresh })) {
    ...
    try {
      ...
      attempts.push(`${source.name}: ${err.message}`);
      console.warn(`[igram] signing chunk from ${source.name} unusable (${err.message})`);
    }
  }

  throw Object.assign(new Error(`no usable igram signing chunk — ${attempts.join(' · ')}`), {
    status: 503,
  });
}
